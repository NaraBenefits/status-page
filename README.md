...
> service (`🛑 <Service> is down`) and assigns `the infrastructure team` to each, and they stay open until
...
closed. Team members in `the infrastructure team` are assigned automatically.
...
2. **DNS** — `status.narabenefits.com` `CNAME` → `avanthealth.github.io`, in
   `an internal Terraform module` in `the internal infrastructure repository`.
